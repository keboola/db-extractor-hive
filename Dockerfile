...
RUN dpkg -i /tmp/hive-odbc.deb || true \
    ...
    && sed -i '/^\[/a DefaultStringColumnLength = 32767' /etc/odbc.ini \
    # Set default maximum string column length for Hive ODBC (driver-level)
    && sed -i '/^\[Cloudera ODBC Driver for Apache Hive 64-bit\]/a DefaultStringColumnLength = 32767' /etc/odbcinst.ini
